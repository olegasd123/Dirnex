...
/// selection unemphasized (gray) when the table lacks focus and emphasized (blue) when it has it,
/// so the color change was the focus theft, not a second highlight.
///
/// Refusing first responder costs nothing the list needs. `mouseDown:` selects rows regardless,
/// so clicking still moves the highlight, and the selection now draws one gray appearance whether
