...
final class PathBarView: NSView, NSTextFieldDelegate {
    ...
    // Internal, not private, for `PathBarView+Editing`: a cross-file extension does not share the
    // type's private scope. Same for the edit-mode state below.
    let crumbStack = NSStackView()
    ...
    var editBase: VFSPath = .local("/")

    // Completion cache: the directory whose children we last fetched, plus those names.
    var completionDirectory: VFSPath?
    var completionChildren: [String] = []
    ...
    private func styleCrumb(_ button: NSButton, isCurrent: Bool) {
        if isCurrent {
            // The user's accent, which resolves to `.controlAccentColor` untouched (PLAN.md §M15).
            button.contentTintColor = isActive
                ? AppPreferences.shared.palette.resolvedAccent
                : .labelColor
    ...
    /// Re-apply the accent after the user changes it (PLAN.md §M15 Slice 2). Internal, unlike the
    /// restyle it wraps, because the pane's palette observer lives in its own file.
