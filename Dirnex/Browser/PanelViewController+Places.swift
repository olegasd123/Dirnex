...
extension PanelViewController {
    ...
    ///
    /// No field-editor carve-out, unlike the ⌃D popup beside it. That one needs one because Cocoa's
    /// `StandardKeyBinding.dict` binds `^d` to `deleteForward:`, so taking the chord would break a
    /// key the text system means something by. The same file binds no `^g` at all (measured), so
    /// there is nothing here to fall through to and the key can mean one thing everywhere.
    @objc func showPlaces(_ sender: Any?) {
        ...
        // Drop it from the path bar's bottom edge whatever its flip orientation, as ⌃D does.
