...
/// The sidebar's half of the user's colors (PLAN.md §M15 Slice 2), shaped as the twin of
...
///
/// The cursor color is the only one of the three that reaches here. The accent belongs to the path
/// bar and the tab strip, and the mark color to a pane's marked rows — a sidebar row is a place,
/// never a file, so it has neither.
extension SidebarViewController {
    /// Every row, so the selected one can wear the cursor color the panes already draw in.
    ...
    /// Subscribe to `paletteDidChange` so the sidebar recolors live. Called once from
    ...
    /// back on the row that carried it, by path, and a recolor must not move the highlight.
