...
final class DirnexRevealScriptCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        return revealed ? true : fail(Scripting.notFound, Scripting.noWindowMessage)
    }
}
...
final class DirnexCopySelectionScriptCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        guard let ran else { return fail(Scripting.notFound, Scripting.noWindowMessage) }
        return ran
    }
}
...
final class DirnexRunOperationScriptCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        guard let query = directParameter as? String else {
            return fail(Scripting.missingParameter, "run operation needs a command id or title.")
        }
        ...
        guard let id = AutomationOperation.resolve(query, userScripts: scripts) else {
            return fail(Scripting.notFound, "\"\(query)\" is not a known Dirnex operation.")
        }
        let ran = MainActor.assumeIsolated { () -> Bool? in
            guard let window = Scripting.activeWindow else { return nil }
        ...
        guard let ran else { return fail(Scripting.notFound, Scripting.noWindowMessage) }
        return ran ? true : fail(Scripting.notFound, "\"\(query)\" could not be run.")
    }
}

// MARK: - Shared

/// Small shared helpers for the scripting handlers: the active window lookup and the AppleScript
/// error numbers, kept in one place so the three commands report failures consistently.
