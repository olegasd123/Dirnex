...
<sup>ttt</sup> Parsed in-process from the same decoded text the source view shows, as a CSV is, so every
location reaches it through the local copy Quick View already works from (<sup>s</sup> for a server).
