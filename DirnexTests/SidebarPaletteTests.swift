...
/// `PanelPaletteTests` when it grew a second pushed color, because the sidebar's row is the one
/// surface that has to serve *three* selection states rather than the panes' two.
///
/// The claim worth pinning is the split itself: the glyph carries the cursor color and the label
/// does not, so an install with a color set draws the same text an untouched one does.
...
struct SidebarPaletteTests {
    ...
    /// *unfocused* selection's glyph in the accent — that is the second place the cursor color
    ...
    func sidebarRowPushesTheForeground() {
        ...
        // Focus moved to a pane — the row keeps AppKit's gray pill and takes the color into its
    ...
    /// take a legible foreground — that color is derived from the cursor, not the cursor itself —
    /// and in every other state it is left to AppKit, so the sidebar's text reads identically whether
    /// or not a color is set. That is the whole distinction between the two pushed values.
    @Test("the sidebar label takes a color only on the filled pill, never the cursor's own")
    func sidebarLabelStaysOutOfThePalette() {
        ...
        // The cursor row: the fill is the cursor color, so the name needs the derived foreground.
        row.isSelected = true
        row.isEmphasized = true
        #expect(cell.labelForeground == .black)
        #expect(cell.textField?.textColor == .black)

        // Focus moved to a pane: the glyph keeps the color, the name hands back to AppKit.
        row.isEmphasized = false
        #expect(cell.glyphForeground == .systemYellow)
    ...
    /// carries (probed), so a pale cursor color left a white house next to a black name. The tell
    /// that the color was baked in rather than requested is that the image stops being a template.
