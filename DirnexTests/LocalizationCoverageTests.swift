    ...
    func everyTourScreenIsTranslated() throws {
        for language in LocalizedBundles.translated {
            ...
            for screen in FirstRunTour.screens {
                ...
                #expect(
                    LocalizedBundles.translation(titleKey, in: bundle) != nil,
                    "\(language.code): no \(titleKey)"
                )
                #expect(
                    LocalizedBundles.translation(bodyKey, in: bundle) != nil,
                    "\(language.code): no \(bodyKey)"
                )
            }
        }
    }
    ...
    func everyScriptTemplateIsTranslated() throws {
        for language in LocalizedBundles.translated {
            ...
            for template in UserScriptTemplate.all {
                ...
                let title = LocalizedBundles.translation(titleKey, in: bundle)
                ...
                #expect(
                    LocalizedBundles.translation(keywordsKey, in: bundle) != nil,
