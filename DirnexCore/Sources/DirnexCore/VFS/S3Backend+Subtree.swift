...
public extension S3Backend {
    ...
    ///
    /// It has nothing partial to offer. A failure part-way through leaves an arbitrary lexicographic
    /// slice of the subtree, which is not "the shallow part" or "the part nearest what you asked
    /// for" — it is wherever the paging happened to stop, so ``SubtreeSearch`` is right to let the
    /// error through rather than present it as a truncated result. Cancellation is the one exception
    /// and it is the caller's own doing: it throws `CancellationError`, which the search converts
    /// back into the stop it asked for, so the two routes are indistinguishable at the Stop button.
