...
public struct VaultLocation: Sendable, Hashable, Codable {
    ...
    /// Only the addressing, deliberately: a vault carries no settings of its own.
    ///
    /// Whether an unlocked vault's volume is visible to the rest of the Mac used to live here, as a
    /// per-vault `showsInFinder`. It is now one app-wide preference
    /// (`AppPreferences.showVaultsInFinder`), so there is one answer to keep and one place to change
    /// it — and nothing here to go stale against it.
    ///
    /// Two consequences worth stating, because both were paid for once. The retired key is still in
    /// every saved vault's JSON and is simply ignored: `JSONDecoder` skips a key no property claims,
    /// which is the *opposite* of the missing-key case that made this type decode by hand in the
    /// first place (docs/NOTES.md ▸ Encryption — a synthesized decoder throws on a key that is not
    /// there, and `SavedVaults` is read through a `try?`, so that would have emptied the Vaults
    /// section). So the synthesized decoder is correct here only while every property below is
    /// required; a new optional one brings the hand-written `init(from:)` back with it.
